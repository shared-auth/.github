...
**Canonical lib-core:** [`shared-auth/shared-auth-lib-core`](https://github.com/shared-auth/shared-auth-lib-core)
**ORM package:** `shared-auth-orm-core` (fold into lib-core; do not keep a second SQL authority)
**Org note:** Auth data planes (`customer-auth` / `admin-auth` RDS). Product SQL for `shared_auth` schema moves into shared-auth-lib-core; shared-defs keeps only a catalog pin.
