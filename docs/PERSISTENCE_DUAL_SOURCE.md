...
**Org:** `shared-auth`  
**Canonical product repos:** ``shared-auth/shared-auth-lib-core``, temporary compatibility ``shared-auth/shared-auth-orm-core` (compatibility only)` (if present)
...
| --- | --- | --- |
| Product relational model (canonical AST) | ``shared-auth/shared-auth-lib-core`` — authored TypeSpec under `contracts/database/typespec/` | Shared-defs product segments; `*-orm-core` hand-written schema |
| Product relational witness (secondary-primary) | ``shared-auth/shared-auth-lib-core`` — independently authored JSON Schema under `contracts/database/json-schema/` | TypeSpec-emitted JSON Schema (witness only) |
| PostgreSQL capabilities not round-trippable | ``shared-auth/shared-auth-lib-core`` — authored `contracts/database/extensions/*.sql` (RLS, grants, triggers, ZD00x guards, specialized indexes, CDC hooks) | ORM macros or generators pretending to emit them |
| Public wire contracts | ``shared-auth/shared-auth-interfaces`` — API/RPC/event TypeSpec + JSON Schema | Persistence tables / ORM entities |
...
High-level move: **SQL generation and ORM code generation leave the shared ORESoftware user/org and live in this GitHub org’s ``shared-auth/shared-auth-lib-core``.** Shared-defs keeps platform SQL, the org→package catalog, Zed locks, and verification harnesses—not this product’s table DDL.
...
| --- | --- | --- | --- |
| Primary | Diesel + diesel-async | Generated from reconciled TypeSpec lineage after dual-source gate | Default for new Rust product code; opaque contexts in ``shared-auth/shared-auth-lib-core`` |
...
- A temporary ``shared-auth/shared-auth-orm-core` (compatibility only)` package may re-export generated Diesel/SeaORM crates for pin compatibility. It must **not** author schema, desired.sql, or a competing generator. Prefer folding into ``shared-auth/shared-auth-lib-core`` `targets.rust-orm` (zed-pkg pattern).
- Cargo features (`read-write`, etc.) express intent; **database principals** (`__api_rw`, `__web_ro`, `__migrator`) are the security boundary.

## 4. Layout inside ``shared-auth/shared-auth-lib-core``
...
Zed pins both authored sources, both generator toolchains, candidates, parity evidence, and release digests. Consumers (`*-api-server.rs`, `*-web-server.rs`, migrator Job) pin the **same** ``shared-auth/shared-auth-lib-core`` digest and select different capability profiles.
...
**Before cutover (bridge):** this org’s segment may still live under `k8s-libs-and-shared-defs/pg-defs/schema/orgs/shared-auth/` and be dual-sourced byte-for-byte into ``shared-auth/shared-auth-lib-core``.

**After cutover:**

- Product DDL is authored/generated only in ``shared-auth/shared-auth-lib-core``.
- Shared-defs catalog entry records `{ package: "shared-auth/shared-auth-lib-core", target: "schema", digest, rdsInstance, postgresSchema|tablePrefix, cdc[] }`.
...
- Edit TypeSpec and authored JSON Schema in ``shared-auth/shared-auth-lib-core`` together; regenerate both candidate pipelines; require parity before publishing release artifacts.
...
- Let ``shared-auth/shared-auth-orm-core` (compatibility only)` or server repos invent schema.
...
## 8. Rollout notes for `shared-auth`

1. Inventory current SQL / SeaORM / shared-defs pins in ``shared-auth/shared-auth-lib-core`` and ``shared-auth/shared-auth-orm-core` (compatibility only)`.
...
6. Retire or demote ``shared-auth/shared-auth-orm-core` (compatibility only)` to a generated compatibility package.

Until dual-source emitters exist, **authored desired-state SQL + extensions in ``shared-auth/shared-auth-lib-core`` remain the interim apply input**, and both TypeSpec and JSON Schema work must be additive toward that file—not a silent second authority in shared-defs.
